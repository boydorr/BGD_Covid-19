...
parms_baseline["ld"]<-1; 
parms_baseline["ld_start"]<-as.Date("2021-04-05")-start_date; parms_baseline["ld_end"]<- as.Date("2021-05-23")-start_date
parms_baseline["ld_improve"] <- 9
parms_baseline["mask"]<-1
parms_baseline["mask_start"]<-as.Date("2021-04-05")-start_date; parms_baseline["mask_end"]<- as.Date("2021-05-23")-start_date
parms_baseline["mask_improve"] <- 9
parms_baseline["mask_compliance"]<-0.2
parms_baseline["syndromic"]<-1
parms_baseline["syn_start"]<-as.Date("2021-04-05")-start_date; parms_baseline["syn_end"]<- as.Date("2021-05-23")-start_date
parms_baseline["community"]<-0.2
...
legend("topleft", c(round(infectious_vec),"data"),title="Initial infectious:",
       col=c("red","orange","blue2","black"), lwd=c(rep(1,length(infectious_vec)),2),cex=1,bty="n")
...
legend("topleft", c(immunity_vec,"data"),title="Initial immune:",bty="n",
       lty=c(2:4,1), lwd=c(rep(1,length(immunity_vec)),2),cex=1)
