...
data <- coronavirus %>% 
          ...
          spread(type, cases) %>% arrange(date) %>%
            ...
            mutate(#case_05 = floor(rollmean(cases, k = 5, fill = NA)),
                   cases_07 = floor(rollmean(cases, k = 7, fill = NA, align="right")),
                   #case_10 = floor(rollmean(cases, k = 10, fill = NA)),
                   cases_14 = floor(rollmean(cases, k = 14, fill = NA, align="right")),
                   #death_05da = floor(rollmean(deaths, k = 5, fill = NA)),
                   deaths_07 = floor(rollmean(deaths, k = 7, fill = NA, align="right")),
                   #death_10 = floor(rollmean(deaths, k = 10, fill = NA)),
...
p1 <- ggplot(data, aes(x=date)) + 
        #annotate("text", x = ymd("2020-05-10"), y = 200, label = "A", size = 6) +
        ...
        #annotate("text", x = ymd("2021-03-22"), y = 200, label = "D", size =8) +
        #geom_segment(aes(x = ymd("2021-03-17"), y = 0, xend = ymd("2021-03-27"), yend = 0, color="10-day nationwide celebrations"), size=2) +
        #geom_segment(aes(x = ymd("2021-03-17"), y = 0, xend = ymd("2021-03-17"), yend = 8000, color="10-day nationwide celebrations"), size=1.1, linetype = "solid") + 
        #geom_segment(aes(x = ymd("2021-03-27"), y = 0, xend = ymd("2021-03-27"), yend = 8000, color="10-day nationwide celebrations"), size=1.1, linetype = "solid") + 
        ...
        #annotate("text", x = ymd("2021-01-28"), y = 2100, label = "B", size =6) +
